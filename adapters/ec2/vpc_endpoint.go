...
func vpcEndpointOutputMapper(_ context.Context, _ *ec2.Client, scope string, _ *ec2.DescribeVpcEndpointsInput, output *ec2.DescribeVpcEndpointsOutput) ([]*sdp.Item, error) {
	...
	for _, endpoint := range output.VpcEndpoints {
		...
		if endpoint.PolicyDocument != nil {
			parsedPolicy, _ := iam.ParsePolicyDocument(*endpoint.PolicyDocument)
		...
		if endpointWithPolicy.PolicyDocument != nil {
			item.LinkedItemQueries = append(item.LinkedItemQueries, iam.LinksFromPolicy(endpointWithPolicy.PolicyDocument)...)
		}
...
func NewVpcEndpointSource(client *ec2.Client, accountID string, region string) *adapters.DescribeOnlySource[*ec2.DescribeVpcEndpointsInput, *ec2.DescribeVpcEndpointsOutput, *ec2.Client, *ec2.Options] {
	return &adapters.DescribeOnlySource[*ec2.DescribeVpcEndpointsInput, *ec2.DescribeVpcEndpointsOutput, *ec2.Client, *ec2.Options]{
		Region:          region,
		Client:          client,
		AccountID:       accountID,
