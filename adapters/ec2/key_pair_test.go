...
func TestKeyPairOutputMapper(t *testing.T) {
	output := &ec2.DescribeKeyPairsOutput{
		KeyPairs: []types.KeyPairInfo{
			{
				KeyPairId:      adapters.PtrString("key-04d7068d3a33bf9b2"),
				KeyFingerprint: adapters.PtrString("df:73:bb:86:a7:cd:9e:18:16:10:50:79:fa:3b:4f:c7:1d:32:cf:58"),
				KeyName:        adapters.PtrString("dylan.ratcliffe"),
				KeyType:        types.KeyTypeRsa,
				Tags:           []types.Tag{},
				CreateTime:     adapters.PtrTime(time.Now()),
				PublicKey:      adapters.PtrString("PUB"),
			},
		},
	}
