...
func capacityProviderOutputMapper(_ context.Context, _ ECSClient, scope string, _ *ecs.DescribeCapacityProvidersInput, output *ecs.DescribeCapacityProvidersOutput) ([]*sdp.Item, error) {
	...
	for _, provider := range output.CapacityProviders {
		attributes, err := adapters.ToAttributesWithExclude(provider, "tags")
		...
		if provider.AutoScalingGroupProvider != nil {
			if provider.AutoScalingGroupProvider.AutoScalingGroupArn != nil {
				if a, err := adapters.ParseARN(*provider.AutoScalingGroupProvider.AutoScalingGroupArn); err == nil {
...
func NewCapacityProviderAdapter(client ECSClient, accountID string, region string) *adapters.DescribeOnlySource[*ecs.DescribeCapacityProvidersInput, *ecs.DescribeCapacityProvidersOutput, ECSClient, *ecs.Options] {
	return &adapters.DescribeOnlySource[*ecs.DescribeCapacityProvidersInput, *ecs.DescribeCapacityProvidersOutput, ECSClient, *ecs.Options]{
		...
		PaginatorBuilder: func(client ECSClient, params *ecs.DescribeCapacityProvidersInput) adapters.Paginator[*ecs.DescribeCapacityProvidersOutput, *ecs.Options] {
