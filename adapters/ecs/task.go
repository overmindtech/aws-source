...
func taskGetFunc(ctx context.Context, client ECSClient, scope string, input *ecs.DescribeTasksInput) (*sdp.Item, error) {
	...
	attributes, err := adapters.ToAttributesWithExclude(task, "tags")
	...
	a, err := adapters.ParseARN(*task.TaskArn)
	...
	if task.ClusterArn != nil {
		if a, err = adapters.ParseARN(*task.ClusterArn); err == nil {
	...
	if task.ContainerInstanceArn != nil {
		if a, err = adapters.ParseARN(*task.ContainerInstanceArn); err == nil {
	...
	if task.TaskDefinitionArn != nil {
		if a, err = adapters.ParseARN(*task.TaskDefinitionArn); err == nil {
...
func taskGetInputMapper(scope, query string) *ecs.DescribeTasksInput {
	...
	return &ecs.DescribeTasksInput{
		...
		Cluster: adapters.PtrString(sections[0]),
...
func tasksListFuncOutputMapper(output *ecs.ListTasksOutput, input *ecs.ListTasksInput) ([]*ecs.DescribeTasksInput, error) {
	...
	for _, taskArn := range output.TaskArns {
		if a, err := adapters.ParseARN(taskArn); err == nil {
...
func NewTaskSource(client ECSClient, accountID string, region string) *adapters.AlwaysGetSource[*ecs.ListTasksInput, *ecs.ListTasksOutput, *ecs.DescribeTasksInput, *ecs.DescribeTasksOutput, ECSClient, *ecs.Options] {
	return &adapters.AlwaysGetSource[*ecs.ListTasksInput, *ecs.ListTasksOutput, *ecs.DescribeTasksInput, *ecs.DescribeTasksOutput, ECSClient, *ecs.Options]{
		ItemType:       "ecs-task",
		Client:         client,
		AccountID:      accountID,
		Region:         region,
		...
		ListFuncPaginatorBuilder: func(client ECSClient, input *ecs.ListTasksInput) adapters.Paginator[*ecs.ListTasksOutput, *ecs.Options] {
