...
func grantOutputMapper(ctx context.Context, _ *kms.Client, scope string, _ *kms.ListGrantsInput, output *kms.ListGrantsOutput) ([]*sdp.Item, error) {
	...
	for _, grant := range output.Grants {
		...
		arn, errA := sources.ParseARN(*grant.KeyId)
		if errA != nil {
			return nil, &sdp.QueryError{
				ErrorType:   sdp.QueryError_OTHER,
		...
		*/
		...
		for _, principal := range principals {
			...
			arn, errA := sources.ParseARN(principal)
			if errA != nil {
...
func NewGrantSource(client *kms.Client, accountID string, region string) *sources.DescribeOnlySource[*kms.ListGrantsInput, *kms.ListGrantsOutput, *kms.Client, *kms.Options] {
	return &sources.DescribeOnlySource[*kms.ListGrantsInput, *kms.ListGrantsOutput, *kms.Client, *kms.Options]{
		ItemType:  "kms-grant",
		Client:    client,
		AccountID: accountID,
		Region:    region,
		...
		InputMapperGet: func(_, query string) (*kms.ListGrantsInput, error) {
			...
			return &kms.ListGrantsInput{
				KeyId:   &tmp[0],                                       // keyID
				GrantId: sources.PtrString(strings.Join(tmp[1:], "/")), // grantId
			}, nil
		},
		UseListForGet: true,
