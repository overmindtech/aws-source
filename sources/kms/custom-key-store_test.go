...
func TestDescribeInput(t *testing.T) {
	tests := []struct {
		name          string
		query         string
		expectedInput *kms.DescribeCustomKeyStoresInput
		expectedError error
	}{
		{
			name:  "ValidIDQueryReturnsCorrectInput",
			query: "id|custom-key-store-1",
			expectedInput: &kms.DescribeCustomKeyStoresInput{
				CustomKeyStoreId: sources.PtrString("custom-key-store-1"),
			},
		},
		{
			name:  "ValidNameQueryReturnsCorrectInput",
			query: "name|custom-key-store-1",
			expectedInput: &kms.DescribeCustomKeyStoresInput{
				CustomKeyStoreName: sources.PtrString("custom-key-store-1"),
			},
		},
		{
			name:  "InvalidQueryFormatReturnsError",
			query: "invalid-query-format",
			expectedError: &sdp.QueryError{
				ErrorType:   sdp.QueryError_NOTFOUND,
				ErrorString: "invalid query format: invalid-query-format, expected 'id|custom-key-store-id' or 'name|custom-key-store-name'",
			},
		},
		{
			name:  "UnknownQueryTypeReturnsError",
			query: "unknown|custom-key-store-1",
			expectedError: &sdp.QueryError{
				ErrorType:   sdp.QueryError_NOTFOUND,
				ErrorString: "invalid query format: unknown|custom-key-store-1, expected 'id|custom-key-store-id' or 'name|custom-key-store-name'",
			},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			input, err := describeInput(tt.query)
			if tt.expectedError != nil {
				...
				if tt.expectedInput.CustomKeyStoreId != nil && *input.CustomKeyStoreId != *tt.expectedInput.CustomKeyStoreId {
					t.Errorf("expected %v, got %v", *tt.expectedInput.CustomKeyStoreId, *input.CustomKeyStoreId)
				}
				if tt.expectedInput.CustomKeyStoreName != nil && *input.CustomKeyStoreName != *tt.expectedInput.CustomKeyStoreName {
					t.Errorf("expected %v, got %v", *tt.expectedInput.CustomKeyStoreName, *input.CustomKeyStoreName)
				}
			}
		})
	}
