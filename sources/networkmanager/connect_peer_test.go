		...
		ConnectPeer: &types.ConnectPeer{
			Configuration: &types.ConnectPeerConfiguration{
				BgpConfigurations: []types.ConnectPeerBgpConfiguration{
					{
						CoreNetworkAddress: sources.PtrString("1.4.2.4"),         // link
						CoreNetworkAsn:     sources.PtrInt64(64512),              // link
						PeerAddress:        sources.PtrString("123.123.123.123"), // link
						PeerAsn:            sources.PtrInt64(64513),              // link
					},
				},
				CoreNetworkAddress: sources.PtrString("1.1.1.3"),  // link
				PeerAddress:        sources.PtrString("1.1.1.45"), // link
			},
			ConnectAttachmentId: sources.PtrString("ca-1"), // link
			ConnectPeerId:       sources.PtrString("cp-1"),
			CoreNetworkId:       sources.PtrString("cn-1"), // link
			EdgeLocation:        sources.PtrString("us-west-2"),
			State:               types.ConnectPeerStateAvailable,
			SubnetArn:           sources.PtrString("arn:aws:ec2:us-west-2:123456789012:subnet/subnet-1"), // link
		},
	}, nil
}
